...
- §25 新增「外部工具（若可用）」小節：執行環境提供 [`zhtw`](https://github.com/sysprog21/zhtw-mcp) MCP 工具時，Agent 於 §23 六個 Pass 後呼叫 `zhtw` 做機械檢查（標點、字形、中國用語），作為語意判斷之外的補充；程式碼與識別名稱不套用其建議，§14.6 與 MUST NOT 優先；工具不存在時略過，不影響本 Skill 既有運作方式。
...
- 台灣用語規範（§11）：中國用語對照、同形異義詞、教育部標準字體用字、全形與半形。
